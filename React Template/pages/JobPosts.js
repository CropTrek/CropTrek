...
export default function JobPosts() {
    ...
    //       console.log("offffffffffffffffffffff", res);
    ...
    async function nbTotalRates(id) {
      try {
        const res = await fetch(`http://localhost:5000/job/countRatingsByCurrentUser/${id}`);
        console.log("brooooooooooooooooooooo", res);
        const data = await res.json(); 
        setTotalRates(data);
      } catch (error) {
        console.error(error);
  ...
  {post.totalRates >= 50 && post.totalRates < 100 && ( 
    <MDBIcon
      fas
      icon="gem"
      size="2x"
  ...
  {post.totalRates >= 100 && post.totalRates < 500 && (
    <MDBIcon
      fas
      icon="gem"
      size="2x"
